...
{
    //讀傳入的網址 UTF8字符串編碼後並執行
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[NSString stringWithCString:url encoding:NSUTF8StringEncoding]]];
}

int ObjCCalls::getStringLength(std::string str)
{
    NSString *strtemp = [NSString stringWithUTF8String:str.c_str()];
    int count = [strtemp length];
    return count;
}

std::string ObjCCalls::getMacAddress()
{
    int                 mgmtInfoBase[6];
    char                *msgBuffer = NULL;
    size_t              length;
