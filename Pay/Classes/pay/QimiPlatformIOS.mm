...
void QimiPlatformIOS::alipayPay(std::string order, float price, std::string product, std::string productDes, std::string appScheme)
{
    /*
     *生成订单信息及签名
     */
	...
	m_order.notifyURL =  @"http://www.baidu.com";                         //回调URL
    
    //应用注册scheme,在AlixPayDemo-Info.plist定义URL types,用于安全支付成功后重新唤起商户应用
	//将商品信息拼接成字符串
	NSString *orderSpec = [m_order description];//NSLog(@"orderSpec = %@",orderSpec);
